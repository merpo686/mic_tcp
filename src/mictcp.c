...
typedef struct buffer_send {
    mic_tcp_payload messages[100];
    int nb_messages;
    int index;
}buffer_send;

struct buffer_send buf;
...
int mic_tcp_connect(int socket, mic_tcp_sock_addr addr)
{
    ...
    if (sock.fd==socket)
    {
        ...
        //4.2: mise en place d'un buffer et du thread envoyeur
        
        buf.nb_messages=0;
        buf.index=0;
        pthread_create(&tid_envoi,NULL,sender,NULL);
        return sock.fd;
    }
    return -1;
}
...
void *sender(void * arg){
    mic_tcp_pdu pdu={0};
    while(1){
        if (buf.nb_messages>0)
        {
            pthread_mutex_lock(&mutex);
            pdu_global.header.source_port=sock.addr.port;
            pdu_global.header.dest_port=addr_dest.port;
            pdu_global.payload.size=buf.messages[buf.index].size;
            pdu_global.payload.data=buf.messages[buf.index].data;
            pthread_mutex_unlock(&mutex);
            ...
            PE=(PE+1)%10;
            pthread_mutex_lock(&mutex);
            buf.index=(buf.index+1)%100;
            buf.nb_messages--;
            pthread_mutex_unlock(&mutex);
        } 
        else
        {
            printf("Rien à envoyer tranquille penard easy posey\n");
            sleep(5);
        }
    }
...
int mic_tcp_send (int mic_sock, char* mesg, int mesg_size)
{
    ...
    pthread_mutex_lock(&mutex);
    if (buf.nb_messages==100){
        printf("trop de messages en attente (100) attendez\n");
    }
    else{
        buf.nb_messages++;
        buf.messages[(buf.nb_messages+buf.index)%100].data=mesg;
        buf.messages[(buf.nb_messages+buf.index)%100].size=mesg_size;
    }
    pthread_mutex_unlock(&mutex);
    return mesg_size;
}
